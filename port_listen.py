...
# 解析命令行参数
parser = argparse.ArgumentParser(description='SMS Server')
parser.add_argument('-p', '--port', type=int, default=11111, help='Port to run the server on')
parser.add_argument('-l', '--maxlen', type=int, default=100, help='Maximum length of the message deque')
args = parser.parse_args()

server = flask.Flask(__name__)

# 使用 deque 存储消息，最大长度通过命令行参数设置
messages = deque(maxlen=args.maxlen)
...
if __name__ == '__main__':
    # 使用 Waitress WSGI 服务器运行，端口通过命令行参数设置
